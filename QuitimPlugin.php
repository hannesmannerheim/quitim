...
class QuitimPlugin extends Plugin {
    ...
    public function onRouterInitialized($m)
    {
        ...
        $m->connect('', array('action' => 'quitimlogin'));      
        $m->connect('main/login', array('action' => 'quitimlogin'));
        $m->connect('main/register', array('action' => 'quitimlogin'));  // only ajax to fight spam
   ...
   /**
    ...
    {
        $action->script($this->path('js/masonry.pkgd.min.js'));      
        $action->script($this->path('js/load-image.min.js'));        
        $action->script($this->path('js/filtrr2-0.6.3.min.js'));
    ...
    /**
     * Auto rotate images and overwrites them http://stackoverflow.com/a/8735110
     *
     */
	...
	function adjustPicOrientation($full_filename, $exif_from_another_file=false){        
		
		if($exif_from_another_file) {
			$exif = $exif_from_another_file;
			}
		else {
