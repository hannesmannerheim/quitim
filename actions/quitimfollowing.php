...
class QuitimFollowingAction extends Action
{
    var $page = null;
    var $notice;
    var $profile;
    ...
    function getFollowersProfiles()
    {
        $profile = array();
        try {
            $subs = Subscription::bySubscriber($this->profile->id,0,10000);
            while($subs->fetch()) {
                $profiles[$subs->subscribed] = $subs->subscribed;
            }
    ...
    function title()
    {
        return sprintf(_("users %s follows"), $this->profile->nickname);
    }
    ...
    function showBody()
    {
        ...
        $this->raw(sprintf(_("users %s follows"), $this->profile->nickname));
		$this->elementEnd('h1');
        $this->elementEnd('a');
